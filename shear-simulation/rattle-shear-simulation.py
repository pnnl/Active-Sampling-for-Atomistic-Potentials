...
# paths
parser.add_argument('--data', type=str, required=True, help='path to dir with data')
parser.add_argument('--model', type=str, required=True, help='path to trained schnet model')
parser.add_argument('--save_path', required=True, type=str, help='path to save output')
parser.add_argument('--schnet_dir', type=str, help='path to schnet dir')
...
sys.path.insert(0, args.schnet_dir)
...
def get_data_name_folder(data_path):
    data_name_folder=None
    if 'pristine' in data_path:
        data_name_folder = 'pristine'
    elif '2Ni' in data_path:
        data_name_folder = '2Ni'
    elif '2Cu' in data_path:
        data_name_folder = '2Cu'
    else:
        found = False
        for letter in ['N','C']:
            for number in ['1','2','3']:
                if letter+number in data_path:
                    data_name_folder = letter+number
                    found = True
                    break
            if found:
                break
    ...
    if 'new' in data_path:
        data_name_folder += '-new'
    elif 'old' in data_path:
        data_name_folder += '-old'
        
    if not data_name_folder:
        raise("No valid name for saving data")
        
    return data_name_folder


##Calculate Metrics##
def calculate_rmsd(ats1, ats2):
    big_ats = ats1.copy()
...
save_path=args.save_path#f'./{model_name}/'
...
# find the folder to save data in
data_name_fldr = get_data_name_folder(args.data)

# make a folder for the final structures and/or traj files
save_structures_path = op.join(save_path, f'{data_name_fldr}')
...
if '.xyz' in args.data:
    ...
    try:
        ...
        raise(f'Something is wrong with the data (vasp?) path you gave. Err: {err}')
...
if args.base:
    ...
    df.to_csv(op.join(save_path, f'{data_name_fldr}.csv'), index=False)


# incorporate slight jitter in the starting positions
for i in range(0,args.rattle):
    ...
    df.to_csv(op.join(save_path, f'{data_name_fldr}-rattle{str(args.starting_iteration+i).zfill(2)}-{args.stdev}.csv'), index=False)