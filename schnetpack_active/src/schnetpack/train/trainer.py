...
class Trainer:
    ...
    def train(self, device, n_epochs=sys.maxsize):
        ...
        scaler = torch.cuda.amp.GradScaler() #16-bit
            ...
            for _ in range(n_epochs):
                ...
                for train_batch in train_iter:
                    ...
                    with torch.cuda.amp.autocast():               #16-bit
                        result = self._model(train_batch)         #16-bit
                        loss = self.loss_fn(train_batch, result)  #16-bit

                    scaler.scale(loss).backward() #16-bit
                    scaler.step(self.optimizer)   #16-bit
                    scaler.update()               #16-bit
                    
                    #Don't forget scalar initialization above the for loop for 16-bit

                    #result = self._model(train_batch)         #32-bit
                    #loss = self.loss_fn(train_batch, result)  #32-bit
                    #loss.backward()                           #32-bit
                    #self.optimizer.step()                     #32-bit
