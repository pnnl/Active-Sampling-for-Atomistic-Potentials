...
class Metric:
    ...
    def give_diffs(self, batch, result):
        raise NotImplementedError
...
class MeanSquaredError(Metric):
    ...
    def give_diffs(self, batch, result):#ADDED TO MAKE IT EASIER TO GET DIFFS IN EVALUATION
        y = batch[self.target]
        if self.model_output is None:
            yp = result
        else:
...
class MeanAbsoluteError(Metric):
    ...
    def give_diffs(self, batch, result):#ADDED TO MAKE IT EASIER TO GET DIFFS FOR EVALUATION
        y = batch[self.target]
        if self.model_output is None:
            yp = result
        else:
...
class AngleRMSE(RootMeanSquaredError):
    ...
    def _get_diff(self, y, yp):
        ...
        diff = torch.clamp(diff, -1, 1)
        angle = torch.acos(diff) / float(3.14159265359) * 180.0
