...
class MBEnvironmentProvider(BaseEnvironmentProvider):
    """
    ...
    def __init__(self, cutoff, angle_cutoff=None): #neighbor_path is a directory to the neigborhood indexes
        self.cutoff = cutoff
        if angle_cutoff == None:
            self.angle_cutoff=cutoff
        else:
    ...
    def get_environment(self, atoms, grid=None):
        ...
        neighbors = np.array([np.pad(x, (0,n_max_nbh-len(x)), mode='constant',
                  constant_values=-1) for x in neighborhood_idx])
        #neighbor_mask = np.zeros_like(neighbors)
        #neighbor_mask[neighbors >= 0] = 1.0
