...
#         try:
#             properties[Properties.neighbors] = row.data['neighborhood_indx'].astype(np.int)
#         except:
#             logging.warning('Neighborhood index not successfully read in')
#             pass
...
class PreprocessedData(AtomsData):
    """
    PyTorch dataset for atomistic data. The raw data is stored in the specified
    ASE database. Use together with schnetpack.data.AtomsLoader to feed data
    ...
    def __init__(
        ...
        # database
        self.dbpath = args.datapath
        self.datapath = '/'.join(args.datapath.split('/')[:-1])
        ...
        logging.info(f'data from cutoff{self.cutoff}')
        self.preprocess_path = op.join(self.datapath, f'cutoff{self.cutoff}') 
    ...
    def get_properties(self, idx, load_only=None):
        ...
        properties = torch.load(op.join(self.preprocess_path, f'{idx}.pt'))

        return None, properties

    def __getitem__(self, idx):
        properties = torch.load(op.join(self.preprocess_path, f'{idx}.pt'))
        properties["_idx"] = torch.LongTensor([idx])

        return properties
    
    def get_metadata(self, key=None):
        """
        Returns an entry from the metadata dictionary (atrefs).
        ...
        Returns:
            value: Full metadata dict.

        """
        atrefs=[[0] for x in range(28)]+[[-5.4],[-3.7]]

        metadata = {'atref_labels': ['energy'], 'atomrefs': atrefs}

        logging.info(f'using atom references {atrefs}')
...
#     except:
#         #logging.warning('The data for neighborhood index in inaccesible')
#         inputs[Properties.neighbors] = None

#     if type(inputs[Properties.neighbors]) == type(None):
#         logging.warning('Calling environement provider')
#          get atom environment
    ...
    inputs[Properties.neighbors] = nbh_idx.astype(np.int)
        #get cell offset
    inputs[Properties.cell_offset] = offsets.astype(np.int)
    ...
    #logging.warning('Inputs have been succesfully prepared')
    return inputs
...
def torchify_dict(data):
    ...
    for pname, prop in data.items():
        #logging.info(prop.dtype)
        #logging.info('Did logging')

        if prop.dtype in [np.int, np.int32, np.int64,torch.int, torch.int32, torch.int64]:
            torch_properties[pname] = torch.LongTensor(prop)
        elif prop.dtype in [np.float, np.float32, np.float64,torch.float, torch.float32, torch.float64]:
