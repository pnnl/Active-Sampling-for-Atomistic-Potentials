...
parser.add_argument('--save_metadata', type=bool,
                    required=False,
                    help='Save metadata in csv file, Default: False',
                   default=False)
args = parser.parse_args()

cutoff = 6
device = 'cuda'
torch_env = TorchEnvironmentProvider(cutoff = cutoff, device = device)



#### Create DFT databases

p = glob.glob("/people/pang248/potential_files/2-DFT/*/")
p.remove('/people/pang248/potential_files/2-DFT/1-pristine-0K/')
...
for data_path in p:
    ...
    dataset = AtomsData(dbpath, available_properties=['energy','forces', 'neighborhood_indx'])
